...
def bank_addUser(account,username,password,counrry,province,street,door):
    ...
    cursor = con.cursor()
    # 查询表中数据
    sql = "select account from jdbcbank"
    # 执行sql
    ...
    if num >= 100:
        ...
        # 正常开户
        sql01 = "select account from jdbcbank where jdbcbank.account = %s"
        param01 = [account]
        ...
        if num == 0:
            # 插入数据
            sql = "insert into jdbcbank values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            param = [account, username, password, counrry, province, street, door, 0, bank_name,]
            # 执行sql
...
def bank_withdrawal(account,password,getmoney):
    ...
    if num == 1:
        sql = "select password from jdbcbank where jdbcbank.account = %s and jdbcbank.password = %s"
...
def bank_transfer(account1,account2,password,money):
    ...
    cursor = con.cursor()
    # 查询表中数据
    sql = "select account from jdbcbank where jdbcbank.account = %s or jdbcbank.account = %s"
    param = [account1,account2]
    # 执行sql
    num = cursor.execute(sql,param)

    if num == 2:
        sql = "select account from jdbcbank where jdbcbank.account = %s and jdbcbank.password = %s"
...
def bank_seek(account,password):
    ...
    cursor = con.cursor()
    # 查询表中数据
    sql = "select * from jdbcbank where account = %s"
    param = [account]
    # 执行sql
    num = cursor.execute(sql,param)
    if num == 1:
